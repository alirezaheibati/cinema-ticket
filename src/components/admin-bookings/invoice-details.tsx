...
export default function InvoiceDetails({ booking }: InvoiceDetailsProps) {
  const ticketsTotal =
    booking?.show?.ticket_price! * booking?.seat_numbers.length!;
      ...
      <div className="bg-gray-100 rounded-sm p-4">
        <div className="grid grid-cols-4 gap-2 py-2">
          <p className="col-span-2 text-sm">
            ...
            <span className="mr-1">
              {addCommasToString(
                convertToPersianDigits(booking?.show?.ticket_price!)
              )}
            </span>
            <span className="text-xs mr-1">تومانی</span>
          </p>
          <p>
            <span>{convertToPersianDigits(booking?.seat_numbers.length!)}</span>
